...
function addCounter(table, idx, name, data){
  ...
  button1.setAttribute("font-size","32px");
  button1.setAttribute("style", 'font-size: x-large;color:#f74c18;border:solid 2px;background-color:black;border-radius:45% ;padding: 9px 21px;text-align:center;')
  ...
  inp.setAttribute("style", "background-color: black;font-size: x-large; color: white;border: none; text-align: center;");
  ...
  button2.innerHTML += "+"
  button2.setAttribute("style", 'font-size: x-large;border:solid 2px;color:#00d586;background-color:black;border-radius:45% ;padding: 8px 17px;text-align:center;')
...
function addText(table, idx, name, data) {
  ...
  //////////////////////////////////////////////////////////
  // danny - was getting black background and black text
  //         on the lenovo tablet. This was in chrome only.
  //         Firefox was ok. Forcing the background to be
  //         white.
  //////////////////////////////////////////////////////////
...
function addRadio(table, idx, name, data) {
  ...
  if (data.hasOwnProperty('choices')) {
    ...
    keys.forEach(c => {
      ...
      inp.setAttribute("name", data.code);
	  inp.setAttribute("style", "top:0; left:0; height:25px; width:25px;border-radious: 50%;accent-color:#f74c18;");
      inp.setAttribute("name", data.code);
...
function addCheckbox(table, idx, name, data){
  ...
  inp.setAttribute("name", data.code);
  inp.setAttribute("style","top: 0; left: 0; height: 25px; width: 25px;;accent-color:#00d586;");
  cell2.appendChild(inp);

  if (data.type == 'bool') {
    cell2.innerHTML += "(check if true)";
  }
...
// danny - new function
function addCheckboxTable(table, idx, name, data){
	
  // row for holding notes 1 and 2
  var row = table.insertRow(idx);
  
   if ((!data.hasOwnProperty('code1')) ||
      (!data.hasOwnProperty('code2')) ||
      (!data.hasOwnProperty('code3')) ||
      (!data.hasOwnProperty('code4')) ||
      (!data.hasOwnProperty('code5')) ||
      (!data.hasOwnProperty('code6')) ||
      (!data.hasOwnProperty('code7')) ||
      (!data.hasOwnProperty('code8')) 
     ) {
    cell1.innerHTML = `Error: Missing code1 through code8 for ${name}`;
    return idx+1;
  }
  ...
  var inp = document.createElement("input");
  inp.setAttribute("id", "input_"+data.code1);
  inp.setAttribute("type", "checkbox");
  inp.setAttribute("name", data.code1);
  inp.setAttribute("style","top: 0; left: 0; height: 25px; width: 25px;;accent-color:#00d586;");
  ...
  var inp = document.createElement("input");
  inp.setAttribute("id", "input_"+data.code2);
  inp.setAttribute("type", "checkbox");
  inp.setAttribute("name", data.code2);
  inp.setAttribute("style","top: 0; left: 0; height: 25px; width: 25px;;accent-color:#00d586;");
  cell1.appendChild(inp);
  cell1.innerHTML += data.title2;


  /////////////////////////////////////
  // row for holding notes 3 and 4
  /////////////////////////////////////
  var row = table.insertRow(idx);
  ...
  cell1.setAttribute("style", "text-align: right;");
  var inp = document.createElement("input");
  inp.setAttribute("id", "input_"+data.code3);
  inp.setAttribute("type", "checkbox");
  inp.setAttribute("name", data.code3);
  inp.setAttribute("style","top: 0; left: 0; height: 25px; width: 25px;;accent-color:#00d586;");
  cell1.appendChild(inp);
  cell1.innerHTML += data.title3;
  ...
  cell1.setAttribute("style", "text-align: left;");
  var inp = document.createElement("input");
  inp.setAttribute("id", "input_"+data.code4);
  inp.setAttribute("type", "checkbox");
  inp.setAttribute("name", data.code4);
  inp.setAttribute("style","top: 0; left: 0; height: 25px; width: 25px;;accent-color:#00d586;");
  cell1.appendChild(inp);
  cell1.innerHTML += data.title4;
  
  /////////////////////////////////////
  // row for holding notes 5 and 6
  /////////////////////////////////////
  var row = table.insertRow(idx);
  ...
  cell1.setAttribute("style", "text-align: right;");
  var inp = document.createElement("input");
  inp.setAttribute("id", "input_"+data.code5);
  inp.setAttribute("type", "checkbox");
  inp.setAttribute("name", data.code5);
  inp.setAttribute("style","top: 0; left: 0; height: 25px; width: 25px;;accent-color:#00d586;");
  cell1.appendChild(inp);
  cell1.innerHTML += data.title5;
  ...
  cell1.setAttribute("style", "text-align: left;");
  var inp = document.createElement("input");
  inp.setAttribute("id", "input_"+data.code6);
  inp.setAttribute("type", "checkbox");
  inp.setAttribute("name", data.code6);
  inp.setAttribute("style","top: 0; left: 0; height: 25px; width: 25px;;accent-color:#00d586;");
  cell1.appendChild(inp);
  cell1.innerHTML += data.title6;
  
  /////////////////////////////////////
  // row for holding notes 7 and 8
  /////////////////////////////////////
  var row = table.insertRow(idx);
  ...
  cell1.setAttribute("style", "text-align: right;");
  var inp = document.createElement("input");
  inp.setAttribute("id", "input_"+data.code7);
  inp.setAttribute("type", "checkbox");
  inp.setAttribute("name", data.code7);
  inp.setAttribute("style","top: 0; left: 0; height: 25px; width: 25px;;accent-color:#00d586;");
  cell1.appendChild(inp);
  cell1.innerHTML += data.title7;
  ...
  cell1.setAttribute("style", "text-align: left;");
  var inp = document.createElement("input");
  inp.setAttribute("id", "input_"+data.code8);
  inp.setAttribute("type", "checkbox");
  inp.setAttribute("name", data.code8);
  inp.setAttribute("style","top: 0; left: 0; height: 25px; width: 25px;;accent-color:#00d586;");
  cell1.appendChild(inp);
  cell1.innerHTML += data.title8;
...
function addElement(table, idx, name, data){
  ...
  } else if (data.type == 'bool2x4')  // danny new "else if" to handle bool2x4 type
  {
    idx = addCheckboxTable(table, idx, name, data);
...
function validateData() {
	...
	for (rf of requiredFields) {
		...
		if (rf == "r") {
			if (!validateRobot()) {
				errStr += rf + " "
				ret = true //false //TODO: change back
			}
		} else if (rf == "l") {
			if (!validateLevel()) {
				errStr += rf + " "
		...
		// Normal validation (length <> 0)
			...
			errStr += rf + " "
			ret = false //false //TODO: change back
		}
	}
	if (ret == false) {
	...
	for (e of inputs) {
		...
		if (radio > -1) {
			...
			if (e.value == "on") {
                ...
                //////////////////////////////////////////////////////////////////////////////////
				// Concatentate the string data. But replace ; with - since ; has special meaning
                //
                // danny - This original code only replaced the first occurence.
...
function qr_regenerate() {
    ...
    // danny - check when this is called.  Maybe we are building it but not updateing the qr code after removing the ; characters
	// Get data
	data = getData()
